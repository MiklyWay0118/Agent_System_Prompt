...
| [Copilot Terminal Tools](https://marketplace.visualstudio.com/items?itemName=mijur.copilot-terminal-tools) | 让 Copilot 能够执行终端命令（编译、运行脚本等）     | **必需** |
| [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python)                             | Python 语言支持（数据生成、对拍脚本运行）           | **必需** |
| [C/Cpp DevTools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpp-devtools)               | C++ 符号导航、调用关系分析等高级功能                | 推荐     |
...
3. **终端工具授权**：安装 Copilot Terminal Tools 后，需允许其执行终端命令的权限。
